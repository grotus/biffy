// $location is used only to redirect to root on successful login by using $location.path('/')
angular.module('biffy').controller('LoginCtrl', ['$scope', '$http', '$location', function($scope, $http, $location) {
    ...
    $http({method: parameters.method,
      ...
      .success(function(data, status){
        if (status == 201 || status == 204){
          ...
          $scope.reset_user();
          if (parameters.success_callback) {
          	parameters.success_callback();
          };
        } else {
          if (data.error) {
